...
static int	ray_same_door_side_wall_axis(t_door_collision *dc, t_dda *dda,
	t_player player, t_fpoint ray_dir)
{
	if (dda->side != dc->door_axis && dc->next_side != dc->door_axis)
	{
		if (dc->door_axis == 1)
			dc->next_wall_y = wall_hitpos(player.y,
					dda->side_dist.x, ray_dir.y);
		else
			dc->next_wall_y = wall_hitpos(player.x,
					dda->side_dist.y, ray_dir.x);
